  ...
  function Row(config, myindex, container) {
      ...
      if(config.rowArrows || config.type === 'seriesSpringboardRow'){
        $.each(config.data, function(index, value){
            $('.'+config.type+myindex+'').append('<div><img id='+config.data[index].id+' src='+config.data[index].image.styles.large+'><div class="duration">'+config.data[index].duration+'</div><div class="label">'+config.data[index].label+'</div></div>');
        });
      }
      else{
          $.each(config.data, function(index, value){
